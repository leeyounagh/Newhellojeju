...
const TravelCommunity = () => {
    ...
    return letter
      .filter((val: any) => {
        if (Search == "") {
          ...
          style === "작성자" &&
          val.writer.name.toLowerCase().includes(Search.toLowerCase())
        ) {
          return val;
        } else if (
          style === "제목" &&
          val.Communutytitle.toLowerCase().includes(Search.toLowerCase())
        ) {
          return val;
        }
      })
  ...
  return (
    <div className="commununity_font">
      ...
      <h3 style={{ position: "absolute", top: "170px", left: "280px" }}>
        최신업데이트
      </h3>
      <div
        style={{
      ...
      >
        <div
          className="clip"
          style={{ width: "700px", height: "100px", display: "flex" }}
        >
          {LatestUpdate()}
        </div>
      </div>
